...
class SphereSaliency():
    def __init__(self, num_drop, num_steps):
        ...
        self.pointclouds_pl, self.labels_pl = MODEL.placeholder_inputs(args.batch_size, None)
    ...
    def drop_points(self, pointclouds_pl, labels_pl, sess):
        ...
        for i in range(self.k):
            ...
            if args.drop_neg:
                sphere_map = np.multiply(np.sum(np.multiply(grad, sphere_axis), axis=2), np.power(sphere_r, args.power))
            else:
                sphere_map = -np.multiply(np.sum(np.multiply(grad, sphere_axis), axis=2), np.power(sphere_r, args.power))
...
def evaluate(num_votes):
    ...
    for fn in range(len(TEST_FILES)):
        ...
        current_data = current_data[:,0:args.num_point,:]
...
if __name__=='__main__':
    with tf.Graph().as_default():
        evaluate(num_votes=args.num_votes)
    LOG_FOUT.close()
